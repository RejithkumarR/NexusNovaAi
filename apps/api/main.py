...
app = FastAPI(title="Nexus Nova AI API", version="0.3.0")
...
def health() -> dict[str, str | bool]:
    ...
    return {"status":"ok","service":"nexus-nova-ai-api","model":model.name,"self_hosted_model":isinstance(model,OllamaModel),"ollama_healthy":ollama_healthy,"rag_configured":bool(settings.vector_db_url)}
...
def chat(request: ChatRequest, _: str = Depends(current_user)):
    ...
    try:
        result=orchestrator.run(RequestContext(text=prompt),max_new_tokens=request.max_new_tokens,temperature=request.temperature)
    except RuntimeError as exc:
        raise HTTPException(status_code=503,detail=str(exc)) from exc
    return {"text":result.text,"capability":result.capability,"model":result.model,"metadata":{**result.metadata,**metadata}}


@app.post("/v1/datasets/upload")
async def upload_dataset(file: UploadFile = File(...), _: str = Depends(current_user)):
    suffix=Path(file.filename or "").suffix.lower()
    if suffix not in SUPPORTED: raise HTTPException(status_code=400,detail="Supported files: .csv, .xlsx, .md")
    root=Path(settings.datasets_dir); root.mkdir(parents=True,exist_ok=True)
    target=root/Path(file.filename or "dataset").name; target.write_bytes(await file.read())
    publisher=GitHubDatasetPublisher.from_environment()
    if publisher: publisher.publish_file(target)
...
def prepare_dataset(_: str = Depends(current_user)):
    try: return DatasetIngestor(Path(settings.datasets_dir)).build_jsonl()
    except (ValueError,OSError) as exc: raise HTTPException(status_code=400,detail=str(exc)) from exc
...
def train(request: TrainRequest, _: str = Depends(current_user)):
    ...
    if not model_name: raise HTTPException(status_code=400,detail="Configure NOVA_TRAINING_MODEL_NAME or NOVA_MODEL_PATH before training")
    dataset=Path(settings.datasets_dir)/"normalized.jsonl"
    if not dataset.exists(): raise HTTPException(status_code=400,detail="Prepare a dataset before starting training")
    try:
        return start_training(model_name=model_name,dataset_path=str(dataset),output_dir=settings.training_output_dir,num_train_epochs=request.epochs or settings.training_epochs)
    except OSError as exc: raise HTTPException(status_code=500,detail=str(exc)) from exc


@app.post("/v1/code/run")
def run_code(request: CodeRunRequest, _: str = Depends(current_user)):
    if request.language.lower() != "python": raise HTTPException(status_code=400,detail="Only Python is enabled in this sandbox release")
    try: result=sandbox.run_python(request.code)
    except (RuntimeError,ValueError) as exc: raise HTTPException(status_code=400,detail=str(exc)) from exc
    return {"stdout":result.stdout,"stderr":result.stderr,"exit_code":result.exit_code,"timed_out":result.timed_out}
